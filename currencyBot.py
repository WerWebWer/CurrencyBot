...
def convert(message):
    ...
    if isfloat(money):
        ...
        msg = 'Получено: ' + str(money) + ' ' + str(def_cur) + '\n\n'
        ...
        msg += 'Для изменения валюты по умолчанию введите /currency'
...
def start(m, res=False):
    ...
    db.db_add_new(m.chat.id, config.DEFAULT_CUR)
...
def status_message(message):
    log(message, message.text)
    def_cur = db.db_get_cur(message.chat.id)
    datas = requests.get('https://api.exchangerate-api.com/v4/latest/' + def_cur).json()
    rates = datas['rates']
    date = datas['date']
    time_last_updated = datas['time_last_updated'] #timestamp
    time = datetime.datetime.fromtimestamp(time_last_updated).strftime("%Y-%m-%d %H:%M:%S")
    ...
    msg = 'Статус по <b>' + str(def_cur) + '</b>:\n\n'
    msg += 'Дата: ' + str(time) + ' GMT\n\n'
    msg += 'Курс:\n'
    msg += 'RUB - <b>' + str(rub) + '₽</b>\n'
    msg += 'USD - <b>' + str(usd) + '$</b>\n'
    msg += 'EUR - <b>' + str(eur) + '€</b>\n'
    msg += 'AMD - <b>' + str(amd) + '֏</b>\n'
    msg += 'HUF - <b>' + str(huf) + 'ƒ</b>\n'
    msg += 'RSD - <b>' + str(rsd) + 'din</b>\n\n'

    bot.send_message(message.chat.id, msg, parse_mode='HTML')
    log_bot(message, 'Send status about ' + def_cur)
...
bot.set_my_commands([
    ...
    telebot.types.BotCommand('/status', 'Вывести информацию о курсе')
])
try:
